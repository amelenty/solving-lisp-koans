...
(defun triangle (&rest x)
    (unless (= (length x) 3)
        (error 'triangle-error))
    (defun list-but-el (l el)
        (let ((n (position el l)))
        (append (subseq l 0 n) (subseq l (+ n 1)))))
    (defun sum-greater-than (l x)
        (> (reduce '+ l) x))
    (defun check-equality (side)
        (sum-greater-than (list-but-el x side) side))
    (defun list-eq-but-el (el)
        (apply #'= (list-but-el x el)))
    (unless (notany #'null (mapcar #'plusp x))
        (error 'triangle-error))
    (unless (notany #'null (mapcar #'check-equality x))
        (error 'triangle-error))
    (when (apply #'= x)
        (return-from triangle :equilateral))
    (when (notevery #'null (mapcar #'list-eq-but-el x))
