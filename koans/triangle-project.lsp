...
(defun triangle (a b c)
    (unless (and (plusp a) (plusp b) (plusp c))
        (error 'triangle-error))
    (unless (and (> (+ a b) c) (> (+ a c) b) (> (+ b c) a))
        (error 'triangle-error))
    (when (= a b c)
        (return-from triangle :equilateral))
    (when (or (eql a b) (eql a c) (eql b c))
        (return-from triangle :isosceles))
    (return-from triangle :scalene))
;  (unless (and (mapcar #'plusp (a b c)))
;    error 'triangle-error)
;  (defun list-but-nth (l n)
;    append (subseq l 0 n) (subseq l (+ n 1)))
;  (defun sum-greater-than (l x)
;    (> (apply '+ l) x))
;  (if (eql a b c)
;    (return :equilateral))
;  (if (or (eql a b) (eql a c) (eql b c));(mapcar #'eql (mapcar #'list-but-nth (a b c) (0 1 2))) ;(eql a b) (eql a c) (eql b c)
;    (return :isosceles))
;  (if (and (sum-greater-than (a b) c) (sum-greater-than (a c) b) (sum-greater-than) (b c) a)
;    (return :scalene))
;  error 'triangle-error)
